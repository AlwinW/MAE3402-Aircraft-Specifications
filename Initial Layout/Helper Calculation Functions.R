...
ClimbRatesFunction <- function(P, Cd0, rho, V, S, K, W) {
  ...
  if (length(sintheta) != 1) {
    ...
    theta = asin(sintheta)
    return(data.frame(
      Theta = theta * 180 / pi,  # NOTE: Theta is in degrees!! theta is in radians
      SinTheta = sintheta, 
...
## Takeoff Ground Roll======================================================================
  ...
  # TR, PR, accel, Int and other values 
  asdf <- LevelRollInt(PTOgr, seq(1e-1,Takeoffout$V2, length.out = resolution * 5))
  
  
  # Testing as I'm not sure I've done this all right. Also for future reference, there should only be ONE
  #   integral call per row, not like 5 haha
  asdf <- asdf %>%
    rowwise() %>%
    mutate(
...
## Climb ======================================================================
  ...
  SteadyClimbInt <- function(inp, h, k) {
    ...
    # I should also return additional values like thrust and power required and acceleration?
    return(Int)
  }
  ...
  integrate(function(x) SteadyClimbInt(POseg4, x, 1.452311)$Eeng, 121.9200, 3048.0000)[[1]]
